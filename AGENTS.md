...
- Home content from MySQL (`force-dynamic`): portfolio, services, technologies. If MySQL is down, those sections render empty instead of crashing.
- After pulling schema changes, run `npm run db:seed` (idempotent per table) so new `services` / `technologies` tables get created and seeded.
